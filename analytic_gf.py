...
NUMBER_GRAMMAR = {
    "a": "<number> -> <digit>",
    "b": "<number> -> <digit> | <number><digit>",
    "c-old": "<number> -> <digit string> | .<digit string>\n<digit string> -> <digit> | <digit string><digit>",
    "c-errata": "<number> -> <digit string> | .<digit string> | <digit string>.<digit string>\n<digit string> -> <digit> | <digit string><digit>",
}
...
def main() -> None:
    ...
    modes = ("a", "b", "c-old", "c-errata") if args.mode == "all" else (args.mode,)
    for mode in modes:
        helper_pairs = []
        number_expr = z
        ...
        eqs = [sp.Equality(lhs, rhs) for lhs, rhs in helper_pairs + grammar_pairs]
        ...
        if helper_pairs:
            ...
            helper_solution = sp.solve(helper_eqs, [lhs for lhs, _ in helper_pairs], dict=True)[0]
        ...
        for lhs, rhs in grammar_pairs:
            print("  ", sp.simplify(lhs), "=", sp.simplify(rhs))
        print("closed N(z) =", sp.factor(number_closed))
        print("closed E(z) =", sp.factor(analytic))
        print("coefficients:")
